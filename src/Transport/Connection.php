...
/*
 * This file has been modified. Search for MODIFIED to find instances of change.
 * File based on: https://github.com/symfony/redis-messenger/blob/5.4/Transport/Connection.php
 */
...
class Connection
{
    ...
    private static function initializeRedisCluster(?\RedisCluster $redis, array $hosts, $auth, int $serializer): \RedisCluster
    {
        ...
        $redis->setOption((string) \Redis::OPT_SERIALIZER, (string) $serializer);
     ...
     * MODIFIED
     * FUNCTION HEADER:
     * The type for $body is string|array changed from just string.
     */
    public function add(string|array $body, array $headers, int $delayInMs = 0): void
    {
        if ($this->autoSetup) {
            $this->setup();
        }
